...
class MapDrawer {
    ...
    updateMap(mapData) {
        this.bounds = {
            x1: Math.min(...mapData.layers.flatMap(layer => layer.dimensions.x.min)) + this.settings.crop_left/this.settings.scale,
            x2: Math.max(...mapData.layers.flatMap(layer => layer.dimensions.x.max)) - this.settings.crop_right/this.settings.scale,
            y1: Math.min(...mapData.layers.flatMap(layer => layer.dimensions.y.min)) + this.settings.crop_top/this.settings.scale,
            y2: Math.max(...mapData.layers.flatMap(layer => layer.dimensions.y.max)) - this.settings.crop_bottom/this.settings.scale
        };
