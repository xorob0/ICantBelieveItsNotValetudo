...
class MapDrawer {
    ...
    updateMap(mapData) {
        this.bounds = {
            x1: Math.min(...mapData.layers.flatMap(layer => layer.dimensions.x.min)) + this.settings.crop_left/this.settings.scale - this.settings.padding_left/this.settings.scale,
            x2: Math.max(...mapData.layers.flatMap(layer => layer.dimensions.x.max)) - this.settings.crop_right/this.settings.scale + this.settings.padding_right/this.settings.scale,
            y1: Math.min(...mapData.layers.flatMap(layer => layer.dimensions.y.min)) + this.settings.crop_top/this.settings.scale - this.settings.padding_top/this.settings.scale,
            y2: Math.max(...mapData.layers.flatMap(layer => layer.dimensions.y.max)) - this.settings.crop_bottom/this.settings.scale + this.settings.padding_bottom/this.settings.scale
        };
    ...
    draw(skipLayers) {
        ...
        const canvasWidth = Math.max.apply(undefined, this.mapData.layers.flatMap(l => l.pixels.filter((_, index) => index % 2 === 0))) + 1 + this.settings.padding_right/this.settings.scale;
        const canvasHeight = Math.max.apply(undefined, this.mapData.layers.flatMap(l => l.pixels.filter((_, index) => index % 2 === 1))) + 1 + this.settings.padding_bottom/this.settings.scale;
